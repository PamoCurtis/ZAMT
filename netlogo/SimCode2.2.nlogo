...
to setup-patches ; cell initiation
  ask patches [
    set cell-density (25 + random 50)
    set pcolor black
    ;set pcolor scale-color violet (45 + random-float 45) 0 100
...
to mutate
  set tumor-size 100 * e ^ (- 3.3 * e ^ (- 0.0054 * ticks)) ; gompertz function
...
to update-color
  ...
  if cell-type = "healthy" [
    set pcolor scale-color pink cell-density 0 100
  ]

  if cell-type = "mutated" [
    set pcolor scale-color red (25 + random-float 25) 0 100
  ]

  if cell-type = "empty" [
    set pcolor scale-color black cell-density 0 100
  ]

  if cell-type = "dead" [
    set pcolor scale-color brown (90 + random-float 5) 0 100
  ]
end
@#$#@#$#@
GRAPHICS-WINDOW
